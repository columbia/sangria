...
impl StatisticsTracker {
    ...
    pub fn get_stats(&mut self) -> HashMap<String, f64> {
        ...
        let waiting_transactions_min = self.waiting_transactions_samples.iter()
            .min()
            .copied()
            .unwrap_or(0);

        let waiting_transactions_max = self.waiting_transactions_samples.iter()
            .max()
            .copied()
            .unwrap_or(0);
        ...
        let waiting_transactions_std = if self.waiting_transactions_samples.len() > 1 {
            let variance = self.waiting_transactions_samples.iter()
                .map(|&x| {
                    let diff = x as f64 - waiting_transactions_avg;
                    diff * diff
                })
                .sum::<f64>() / (self.waiting_transactions_samples.len() - 1) as f64;
            variance.sqrt()
        } else {
            0.0
        };
        ...
        stats.insert("waiting_transactions_avg".to_string(), waiting_transactions_avg);
        stats.insert("waiting_transactions_min".to_string(), waiting_transactions_min as f64);
        stats.insert("waiting_transactions_max".to_string(), waiting_transactions_max as f64);
        stats.insert("waiting_transactions_std".to_string(), waiting_transactions_std);
        stats.insert("waiting_transactions_count".to_string(), self.waiting_transactions_samples.len() as f64);
        stats
    }
